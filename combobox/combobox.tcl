# https://github.com/jcowgar/misctcl/blob/master/combobox/combobox.tcl

package provide mpcombo 1.0
...
proc ::ttk::combobox::CompleteList { W key { start -1 } } {
  ...
  after 350 {set ::ttk::combobox::keyaccumulator ""}

  # if { $start == -1 } {
  #   set start [expr { [$W curselection] + 1 }]
  # }
  for { set idx $start } { $idx < [$W size] } { incr idx } {
    if { [string match -nocase $key* [string trim [$W get $idx] ']] } {
